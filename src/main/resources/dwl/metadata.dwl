...
(vars.metadata map ((item, index) ->{
    ...
    IMSRTX: trim(item.IMSRTX),
    IMDRAW: trim(item.IMDRAW),
    ...
    Detail_Product_Description__c : trim(item.DRDL01_SRP2),
    //Detail_Product_Description__c : concatKeyValue(trim(item.DRKY_SRP2),(item.DRDL01_SRP2),""),
    General_Product_Type__c: concatKeyValue(trim(item.DRKY_SRP1),trim(item.DRDL01_SRP1),""),       
    General_Product_Code__c : trim(item.DRKY_SRP1),
