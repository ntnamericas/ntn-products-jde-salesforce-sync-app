...
COLEDG07_F4105 AS (
 ---Based on changed since last run (review  COUMPJ and COTDAY)Hold variable COLITM
    ...
    WHERE  TRIM(X1.COLEDG)='07' AND TRIM(X1.COMCU)='1801' AND (TRIM(X1.COUPMJ) >= $(vars.previouscifJobRun.date) AND TRIM(X1.COTDAY) >= $(vars.previouscifJobRun.time))
	--WHERE TRIM(X1.COLITM)='592A[M100]' 
...
WHERE (T5.COUPMJ >= $(vars.previouscifJobRun.date)) AND (T5.COTDAY >= $(vars.previouscifJobRun.time)) AND TRIM(T5.COMCU)='1801'
