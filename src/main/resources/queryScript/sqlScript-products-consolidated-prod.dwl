...
IBLTLV_IBLITM AS (
    ...
    FROM PRODDTA.F4102 Y2
    --WHERE trim(Y2.IBMCU) = '1801'
    --WHERE (TRIM(Y2.IBUPMJ) >= $(vars.previousProductsJobRun.date) AND TRIM(Y2.IBTDAY) >= $(vars.previousProductsJobRun.time)) and trim(Y2.IBMCU) = '1801'
	WHERE ( Y2.IBUPMJ > $(vars.previousProductsJobRun.date) OR (Y2.IBUPMJ = $(vars.previousProductsJobRun.date) AND Y2.IBTDAY >= $(vars.previousProductsJobRun.time))) AND TRIM(Y2.IBMCU) = '1801'
)
...
SELECT
    T1.IBUPMJ, T1.IBTDAY, T2.IMUPMJ, T2.IMTDAY, T12.DRUPMJ, T12.DRUPMT,
    T1.IBPRP1,
    T1.IBMCU,
    T1.IBPRP4,
    T1.IBLITM,
...
GROUP BY
    ...
    T1.IBPRP7, T1.IBUPMJ, T1.IBTDAY, T2.IMUPMJ, T2.IMTDAY, T12.DRUPMJ, T12.DRUPMT
	--T16.IMDRAW, T16.IMSRTX, T17.IMLITM,"
